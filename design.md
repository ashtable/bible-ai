...
> ⚠️ **Task 0' finding (pre-confirmed, 2026-06-22):** `StableDiffusionPipeline(resourcesAt:)` expects a directory of compiled **`.mlmodelc`** bundles, not `.mlpackage` files. `coremltools` produces `.mlpackage`; `--bundle-resources-for-swift-cli` compiles them to `.mlmodelc`. Required files: `TextEncoder.mlmodelc`, `Unet.mlmodelc`, `VAEDecoder.mlmodelc`, `VAEEncoder.mlmodelc`, `vocab.json`, `merges.txt`. `ModelLocator` (Task 9b) must resolve a model ID to this `.mlmodelc` directory, not a `.mlpackage` file.
>
> ✅ **Task 0' confirmed on hardware (2026-06-28, iPhone 17 Pro / iOS 27):** SD 1.5 exports, side-loads, and runs end-to-end. Findings for Tasks 12/13:
> - **Initializer:** `StableDiffusionPipeline(resourcesAt:controlNet:configuration:disableSafety:reduceMemory:)`. The `resourcesAt:` overload requires an explicit `controlNet:` argument (pass `[]` for text-to-image); there is no `controlNet`-less overload in 1.1.x. `MLModelConfiguration.computeUnits = .cpuAndNeuralEngine`.
> - **Progress:** `progressHandler` (`(Progress) -> Bool`) fires once per denoising step; `progress.step` is **0-indexed** (`timeSteps.enumerated()`), `progress.stepCount` is `timeSteps.count`. Return `true` to continue, `false` to cancel — confirmed.
> - **Scheduler / step count:** the **default PNDM scheduler appends a duplicate PLMS warm-up timestep**, so it emits `stepCount + 1` callbacks (21 for a requested 20). Use **`.dpmSolverMultistepScheduler`** to run exactly `stepCount` steps; it is also higher quality at the low step counts the ≤15s gate targets. Tasks 12/13 should default to DPM-Solver.
> - **Timing:** 20-step 512×512 inference = **8.3s warm** (well under the 15s gate). First-ever run is ~40s due to one-time Apple Neural Engine model compilation, which the OS caches on disk (and is invalidated when the app binary is reinstalled). Real-world routing should treat the first generation after install as a one-time warm-up.
> - **Model source:** `CompVis/stable-diffusion-v1-5` was deleted from Hugging Face; the spike used `Lykon/dreamshaper-8` (public SD 1.5 fine-tune, identical architecture). `ModelLocator` must not hardcode the CompVis repo.
> - **Side-load:** models live at `Application Support/Models/sd-1-5/`. Pushing via `xcrun devicectl device copy to` of the whole `Resources/` directory recurses correctly; copying a *file* onto an existing *directory* destination replaces the directory (a per-file push corrupts the `.mlmodelc` bundles). See `scripts/export_sd15_coreml.sh`.
...
3. **Task 0 hard blocker — CLEARED.** Task 0 spike (complete): `CoreAIDiffusionPipeline` absent from iOS 27 SDK — confirmed negative. On-device image re-scoped to Core ML (`.mlmodelc`). **Task 0' (complete, 2026-06-28):** SD 1.5 exports via `coremltools` to `.mlpackage`, compiles to `.mlmodelc`, and runs **20-step 512×512 inference in 8.3s warm on iPhone 17 Pro / iOS 27** — under the 15s gate. Image-generation UI (Tasks 12/13) is unblocked; see the §6.6 Task 0' confirmation block for the API/scheduler/timing details.
